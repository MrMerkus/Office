---
name: codex-fleet
description: Standalone Codex CLI runner + fleet orchestrator. Does THREE things and always EXECUTES them (never just describes): (1) general code tasks via `codex exec`, (2) high-quality image generation via Codex's built-in `gpt-image-2` tool, and (3) parallel multi-lane fleets — spawning many `codex exec` delegates at once with worktree isolation. Defaults locked: model `gpt-5.6-luna`, reasoning `high`, `--skip-git-repo-check` always. For multiple independent jobs, fire them ALL in parallel — compute is not the constraint, throughput is. Triggers on: "use codex", "run codex", "codex exec", "imagegen", "generate image", "make image", "render this", "ask codex to ...", "have codex ...", "spawn a fleet", "parallel codex", any image-asset request (icons/sigils/banners/portraits/backgrounds/sprites/UI assets/mockups/photoreal/etc.), and any request to delegate code-level work to Codex.
---

# Codex Fleet — Standalone Action Runner

## Yerel gerçekler (kullanıcının makinesi — 2026-09-10'da doğrulandı)

Bu blok yukarı akıştaki metni **ezer**. Çelişki olursa burası geçerlidir.

| Konu | Yukarı akış | Burada doğru olan |
|---|---|---|
...
|---|---|---|
| Model | `gpt-5.6-luna` | `-m <model>` only if user specifies |
...
```
Bash tool call:
  command: codex exec --skip-git-repo-check -m gpt-5.6-luna \
           -c model_reasoning_effort=high \
           --sandbox read-only \
...
- `-m gpt-5.6-luna` — agent model that decides to call `image_gen`. Best prompt-following for image workflows.
...
> **Keeping the machine awake (Linux).** An idle suspend silently kills a mid-flight lane (you'll see ~5KB of output, zero edits). The upstream skill used macOS `caffeinate -i`; here, wrap a long fleet in `systemd-inhibit --what=idle:sleep --why='codex fleet' <cmd>`, or just keep the session active. A respawn with the same brief is safe when `git status` shows no partial work.
